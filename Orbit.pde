...
void updateHelper(Orbiter planets, double timeDelta){ 
    planets.updateRotation(timeDelta); 
    for(Orbiter o : planets.getChildren()){ 
      updateHelper(o, timeDelta);
    }
}

void updateOrbiters(double timeDelta) {
    updateHelper(sun, timeDelta);
}
...
void helperPDE(Orbiter planets){ 
  drawOrbiter(planets);
