...
/**
 * An Orbiter is an object that orbits some other object, called its parent. The center of an
 * orbital system is an Orbiter with no parent. Each Orbiter may have child Oribters that orbit it.
 *
...
public class Orbiter {
  ...
  public Orbiter(Orbiter parent, double orbitRadius, double orbitAngle, double orbitSpeed,
      Type type, Color fillColor) {
  ...
  public Matrix getMatrix() throws UndefinedMatrixOpException {
    ...
    // do this). Make sure you've coded the Matrix class first.
      ...
      // Roatated
      Matrix RotatedMatrix = Matrix.rotationH2D(this.orbitAngle);
      Matrix TranlsatedMatrix = Matrix.translationH2D(this.orbitRadius, 0);
      // Translation
      Matrix transformedMatrix = RotatedMatrix.dot(TranlsatedMatrix);
      return parent.getMatrix().dot(transformedMatrix);
